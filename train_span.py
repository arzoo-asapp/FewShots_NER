...
from utils import pprint, ensure_path, Averager, Timer, count_acc, pad_sentences, euclidean_metric, read_data, simplify_token, save_dev_span_output, count_F, Aggregate_F
...
def do_test_pass(test_batches_shot, test_batches_query, test_batches_labels, shot, way, query, expressions, train, test, id_to_token=None, id_to_tag=None, test_cls=None):
    ...
    for i, (batch_shot, batch_query, batch_labels) in enumerate(zip(test_batches_shot, test_batches_query, test_batches_labels), 1):
        data_token_shot = [x for _, x, _, _ in batch_shot]
        data_sentence_shot = [sent for sent, _, _, _ in batch_shot]
        ...
        data_token_query = [x for _, x, _, _ in batch_query]
        data_sentence_query = [sent for sent, _, _, _ in batch_query]
        ...
        for batch_index in np.arange(0, len(batch_labels), block_size):
            #print(batch_index)
            ##exit()
            ...
            (mini_data_sentence_query, mini_sentence_query_lens)= pad_sentences(mini_data_sentence_query, MAX_SENT_LEN)

            #labels = batch_labels
            #print(len(labels))
        
            mini_labels = batch_labels[batch_index: batch_index+block_size]
            #print(len(labels))
            mini_labels = torch.LongTensor(np.array(mini_labels)).to(device)
            #print(labels)
        
            logits = euclidean_metric(model(mini_data_sentence_query, mini_data_token_query, mini_sentence_query_lens), proto)
            ...
            if test:
                ...
                save_dev_span_output(output_file, logits, mini_labels, mini_labels, mini_data_sentence_query, mini_data_token_query, mini_sentence_query_lens, id_to_token, id_to_tag)
            correct, total_preds, total_gold = count_F(logits, mini_labels, set(batch_labels))
            #print(correct)
            #print(total_preds)
            #print(total_gold)
...
def main():
    ...
    trainset = SpanDataset(train, token_to_id, id_to_token, tag_to_id, id_to_tag, 'train', args.test_class, args.val_class)
    ...
    valset = SpanDataset(dev, token_to_id, id_to_token, tag_to_id, id_to_tag, 'dev', args.test_class, args.val_class)
    val_sampler = CategoriesSampler_Test(valset.data_label, valset.data_sent_id, N_VAL_BATCHES, args.test_way, args.shot, args.query, True)
    ...
    testset = SpanDataset(test, token_to_id, id_to_token, tag_to_id, id_to_tag, 'test', args.test_class, args.val_class)
    test_sampler = CategoriesSampler_Test(testset.data_label, testset.data_sent_id, N_TEST_BATCHES, args.test_way, args.shot, args.query, True)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        model.eval()
        _, val_acc = do_test_pass(val_batches_shot, val_batches_query, val_batches_query_labels, args.shot, args.test_way, args.query, expressions, False, False, id_to_token, id_to_tag, args.test_class)
